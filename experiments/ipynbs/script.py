...
variables = {
    "method": [
        "Prune by CI",
        "Prune by CI with Head Pruning",
        "Prune by CI(attention only)",
        "Prune by CI(feedforward only)",
        "Prune by Generated samples(feedforward only)",
        "Prune by Head Pruning only",
        "Prune by magnitude",
        "Prune by wanda",
    ],
    "model": [
...
for method in variables["method"]:
    for model in variables["model"]:
        for ratio in variables["ratio"]:
            file_name = f"experiments/ipynbs/{method}/{model}/{method}({ratio}).ipynb"
